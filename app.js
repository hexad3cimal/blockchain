...
app.use(function(req, res, next) {
	...
	if (req.originalUrl.indexOf('/users') >= 0 || req.originalUrl.indexOf('/validate') >= 0 ) {
		return next();
	}
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', async function(req, res) {
	...
	t = t.replace(/'/g, '"');
	t = JSON.parse(t);
	let message1= await query.queryChaincode(peers[0], channelName, chaincodeName, t, 'query', req.username, req.orgname);
let resp;
	request.post('http://localhost:4000/validate', {
  json: {
    value: 'Buy the milk'
  }
}, (error, res, body) => {
